...
class Point(object):
    ...
    def move_by(self,dx,dy):
        self.distance = self.distance + math.sqrt(((self.x + dx)-self.x)**2
                                                  + ((self.y + dy)-(
            self.y))**2)
        self.x = self.x + dx
        self.y = self.y + dy
    ...
    def get_distance_from(self,p2):
        self.distance = math.sqrt((p2.y - self.y)**2 + (p2.x  - self.x)**2)
        return self.distance

    def get_distance_from_start(self):
        self.distance = math.sqrt((self.x - self.start_x)**2 + (self.y -
                                                                self.start_y)**2)
        return self.distance
